...
class InferenceModule:
    """
    An inference module tracks a belief distribution over a ghost's p.
    ...
    def getBeliefDistribution(self):
        ...
        ps conditioned on all evidence so far.
        """
        pass
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: #special case 1
            self.particles = [self.getJailPosition() for p in range(self.numParticles)]
        else:
            allPossible = util.Counter()
            oldBelief = self.getBeliefDistribution()

            for legalPosition in self.legalPositions:
                distance = util.manhattanDistance(pacmanPosition, legalPosition)
                allPossible[legalPosition] += emissionModel[distance] * oldBelief[legalPosition]
            ...
            if allPossible.totalCount() == 0: #special case 2
                ...
                counter = 0
                while counter < self.numParticles:
                    newDist.append(util.sample(allPossible))
                    counter += 1
    ...
    def getBeliefDistribution(self):
        ...
        ps conditioned on all evidence and time passage. This method
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        possiblePositions = list(itertools.product(positions, repeat = repeat))
    ...
    def observeState(self, gameState):
        ...
        if allPossible.totalCount() == 0:
            ...
            counter = 0
            while counter < self.numParticles:
                newDist.append(util.sampleFromCounter(allPossible))
                counter += 1
            self.particles = newDist
